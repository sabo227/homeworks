object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        def all(n: Int, div: Int): List[Int] = {
            if (n == 1) Nil
            else if (div * div > n) List(n)
            else if (n % div == 0)  div :: all(n / div, div)    
            else all(n, div + 1)
        }

        all(number, 2).distinct
    }
    ...
    def sumScalars(leftVec0: Vector2D, leftVec1: Vector2D, rightVec0: Vector2D, rightVec1: Vector2D) =
        sumByFunc(leftVec0, leftVec1, scalar, rightVec0, rightVec1)

    def sumCosines(leftVec0: Vector2D, leftVec1: Vector2D, rightVec0: Vector2D, rightVec1: Vector2D)=
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val weight = (tuple: (Int, Double)) => 4 / 3 * java.lang.Math.PI * math.pow(tuple._1, 3) * tuple._2
